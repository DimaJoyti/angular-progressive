...
const BASE_URL = 'http://localhost:4200/widgets/';
...
export class WidgetsService {
  ...
  add(widget: Widget){
    ...
    return this.http.post(BASE_URL, JSON.stringify(widget))
    .map(res => res.json())
    .do(data => {
  ...
  update(widget: Widget, update){
    ...
    return this.http.put(`${BASE_URL}?id=${widget.id}`, JSON.stringify(update))
    .map(res => res.json())
    .do(updated => {
